...
- **배포:** GitHub Pages — `https://96thd.github.io/portfolio/` (`main` 브랜치 루트를 그대로 서빙).
- **로컬 확인:** 정적 서버면 충분. 예: `python3 -m http.server 8000` 후 `http://localhost:8000/`.
  (모듈 로딩·fetch 때문에 `file://` 직접 열기는 일부 기능이 동작하지 않음.)
...
pdf.html              PDF 생성 전용 페이지 (Actions에서 Puppeteer가 렌더 → portfolio.pdf)
...
.github/workflows/build-pdf.yml   works.js/pdf.html 변경 시 PDF 재생성 후 [skip ci] 커밋
```
...
- **`portfolio.pdf`는 직접 편집하지 않는다.** `works.js` 또는 `pdf.html`을 고치면 Actions가 재생성한다.
- **`index.html`의 `<script>` 로드 순서를 바꾸지 않는다.** `works.js` → `core.js` → 나머지 → `events.js`(루프 시작).
- 작업물 추가/수정은 `js/works.js` 배열만 편집하면 메인 갤러리와 PDF 양쪽에 반영된다.
- 다크/라이트 양쪽을 지원한다. 색은 `css/style.css` 상단 토큰과 `core.js`의 `isLight` 분기를 함께 확인.
- 주석은 한국어로 작성되어 있다. 기존 스타일(간결한 한국어 설명, 구분선 배너)을 따른다.
- 배포는 `main`에 push하면 자동. 별도 빌드 단계 없음.
