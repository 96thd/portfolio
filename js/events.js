...
(function () {
  ...
  addEventListener('popstate', () => {
    ...
    if (v === 'gallery') {
      // 인트로에서 앞으로가기 → 인트로를 부드럽게 완료
      if (introActive() && window.introAPI) window.introAPI.complete();
    } else {
      // 갤러리에서 뒤로가기 → 인트로 재진입
      if (!introActive() && window.introAPI) window.introAPI.reenter();
    }
  });
  ...
  // iframe 주소 변경. src 대입은 그 자체로 히스토리 항목을 만들어
  // (열 때 1개 + 닫을 때 1개) 뒤로가기가 유령 항목에 걸린다.
  // location.replace()는 항목을 남기지 않는 이동이라 히스토리가 깨끗하게 유지됨.
  function setIframe(url) {
    const f = $('modal-iframe');
    if (!f) return;
    try { f.contentWindow.location.replace(url); return; } catch (e) {}
    f.src = url;   // 접근 불가 시 폴백
  }
 ...
 /* ─── card click → modal ─── */
  ...
  function openVideo(i) {
    ...
    setIframe(`${YT_ORIGIN}/embed/${works[i].id}?autoplay=1&enablejsapi=1&origin=${location.origin}`);
  ...
  window.closeModal = function (e) {
    ...
    if (curView() === 'video') {
      history.back();
      // 안전망: iframe 등이 히스토리를 오염시켜 back()이 헛돌면 직접 닫음
