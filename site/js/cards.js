...
(function () {
  ...
  function makeImageCard(card, w) {
    ...
    bd.style.cssText = 'position:absolute;inset:0;border-radius:12px;border:1px solid rgba(255,255,255,.07);pointer-events:none;box-sizing:border-box;';
    card.appendChild(bd);
    card._bd = bd;
  }
  ...
  App.drawCards = function (frac, hP, f) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (isCenter !== card._lastCenter) {
        ...
        if (isCenter) {
          card._bd.style.border  = isLight ? '2px solid rgba(20,18,16,.30)' : '2px solid rgba(255,255,255,.62)';
          card.style.boxShadow   = isLight ? '0 0 40px rgba(0,0,0,.06)' : '0 0 40px rgba(255,255,255,.08)';
        } else {
          card._bd.style.border  = isLight ? '1px solid rgba(20,18,16,.06)' : '1px solid rgba(255,255,255,.07)';
